...
Write routes (POST/PATCH/DELETE) require a JWT — see test_auth.py for the
dedicated auth-enforcement tests. Here, write-path tests use the `auth_headers`
fixture (a real token from a real login) so the tests stay focused on route
behaviour rather than re-proving auth works on every single test.
...
class TestStudentsRoutes:
    ...
    def test_post_student_creates_and_returns_201(self, client, auth_headers):
    ...
    def test_post_student_rejects_empty_name(self, client, auth_headers):
        # Pydantic validation: name has min_length=1 in schemas.py
        response = client.post(
            "/students",
            json={"name": "", "grade_level": "5A"},
            headers=auth_headers,
        )
        assert response.status_code == 422

    def test_post_student_rejects_missing_grade_level(self, client, auth_headers):
        response = client.post(
            "/students", json={"name": "Alice"}, headers=auth_headers
        )
        assert response.status_code == 422

    def test_get_students_returns_all_created_students(self, client, auth_headers):
        client.post(
            "/students",
            json={"name": "Alice", "grade_level": "5A"},
            headers=auth_headers,
    ...
    def test_get_student_by_id_returns_the_student(self, client, auth_headers):
        created = client.post(
            "/students",
            json={"name": "Alice", "grade_level": "5A"},
            headers=auth_headers,
    ...
    def test_patch_student_updates_only_sent_fields(self, client, auth_headers):
        created = client.post(
            "/students",
            json={"name": "Alice", "grade_level": "5A"},
            headers=auth_headers,
    ...
    def test_patch_student_returns_404_when_not_found(self, client, auth_headers):
        response = client.patch(
            "/students/999", json={"name": "Ghost"}, headers=auth_headers
        )
        assert response.status_code == 404

    def test_delete_student_returns_204_and_removes_it(self, client, auth_headers):
        created = client.post(
            "/students",
            json={"name": "Alice", "grade_level": "5A"},
            headers=auth_headers,
    ...
    def test_delete_student_returns_404_when_not_found(self, client, auth_headers):
        response = client.delete("/students/999", headers=auth_headers)
        assert response.status_code == 404
    ...
    def test_student_summary_returns_empty_subjects_for_new_student(
...
class TestSubjectsRoutes:
    ...
    def test_post_subject_creates_and_returns_201(self, client, auth_headers):
        response = client.post(
    ...
    def test_post_subject_rejects_empty_name(self, client, auth_headers):
        response = client.post(
            "/subjects", json={"name": ""}, headers=auth_headers
        )
        assert response.status_code == 422
...
class TestGradesRoutes:
    ...
    @staticmethod
    def _make_student_and_subject(client, auth_headers):
        student = client.post(
            "/students",
            json={"name": "Alice", "grade_level": "5A"},
            headers=auth_headers,
    ...
    def test_post_grade_creates_and_returns_201(self, client, auth_headers):
    ...
    def test_get_grades_no_filters_returns_all(self, client, auth_headers):
        student_id, subject_id = self._make_student_and_subject(client, auth_headers)
    ...
    def test_get_grades_filtered_by_student(self, client, auth_headers):
        alice = client.post(
            "/students",
            json={"name": "Alice", "grade_level": "5A"},
            headers=auth_headers,
...
class TestEndToEndFlow:
    ...
    def test_create_student_subject_grades_then_check_summary(
        self, client, auth_headers
    ):
        student = client.post(
            "/students",
